...
function onSearch(obj){//js函数开始
    setTimeout(function(){//因为是即时查询，需要用setTimeout进行延迟，让值写入到input内，再读取
        ...
        var searchCol = 2;//要搜索的哪一列，这里是第二列，从0开始数起
...
function modify(obj){
    ...
    var oVideo_Name = document.getElementById('Video_Name');
    var oVideo_Introduction = document.getElementById('Video_Introduction');
    var oVideo_address = document.getElementById('Video_address');
    var oTr = obj.parentNode.parentNode;
    var aTd = oTr.getElementsByTagName('td');
    rowIndex = obj.parentNode.parentNode.rowIndex;
    oId.value = aTd[0].innerHTML;
    oLearning_direction.value = aTd[1].innerHTML;
    oVideo_Name.value = aTd[2].innerHTML;
    oVideo_Introduction.value = aTd[3].innerHTML;
    oVideo_address.value = aTd[4].innerHTML;
...
function update(){
    ...
    otable.rows[rowIndex].cells[2].innerHTML = oVideo_Name.value;
    otable.rows[rowIndex].cells[3].innerHTML = oVideo_Introduction.value;
    otable.rows[rowIndex].cells[4].innerHTML = oVideo_address.value;
}